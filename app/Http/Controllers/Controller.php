...
abstract class Controller extends BaseController {
	...
	function getPacaut( Request $request)
	{
		$q=$request->input('q');
		$p=$request->input('p');
		$rows = $this->model->getPacaut($q, $p);
		$items = array();
		...
		foreach($rows as $row)
		{
			...
			$nombres='nombres';
			$apellidos='apellidos';
			$fechaSolicitudIPS='fechaSolicitudIPS';
			$fechaSolicitudOM='fechaSolicitudOM';
			$nombreServicio='nombreServicio';
			$estadoPaciente='estadoPaciente';
			$fechaEsperada='fechaEsperada';
			$FechaReal='FechaReal';
			$numAutorizacion='numAutorizacion';
			$ipsAsignada='ipsAsignada';
			$fechaAutorizacion='fechaAutorizacion';
			$fechaVencimiento='fechaVencimiento';
			$tb_estadoAutorizacion_id='tb_estadoAutorizacion_id';
			$fechaCita='fechaCita';
			$nroFactura='nroFactura';

			$items[] = array('tipoDoc'=>$row->$tipoDoc,
					'numDoc'=> $row->$numDoc,
					'nombres'=> $row->$nombres,
					'apellidos'=> $row->$apellidos,
					'fechaSolicitudIPS'=> $row->$fechaSolicitudIPS,
					'fechaSolicitudOM'=> $row->$fechaSolicitudOM,
					'nombreServicio'=> $row->$nombreServicio,
					'estadoPaciente'=> $row->$estadoPaciente,
					'fechaEsperada'=> $row->$fechaEsperada,
					'FechaReal'=> $row->$FechaReal,
					'numAutorizacion'=> $row->$numAutorizacion,
					'ipsAsignada'=> $row->$ipsAsignada,
					'fechaAutorizacion'=> $row->$fechaAutorizacion,
					'fechaVencimiento'=> $row->$fechaVencimiento,
					'tb_estadoAutorizacion_id'=> $row->$tb_estadoAutorizacion_id,
	...
	function getDocpaciente( Request $request)
	{

		if($request->ajax() == true && \Auth::check() == true)
		{
			$q=$request->input('q');
			$rows = $this->model->getDocpaciente($q);
			$items = array();
			...
			foreach($rows as $row)
			{
				...
				$nombres = "nombres";
				$apellidos = "apellidos";
				$items[] = array('id'=>$row->$id,'text'=> 'Documento:'.$row->$tipoDoc.' '.$row->$numDoc.' Nombres:'.$row->$nombres.' '.$row->$apellidos);
			}
	...
	function getCargarmes( Request $request)
	{

		if($request->ajax() == true && \Auth::check() == true)
		{
			$result = $this->model->cargarMesAnterior();
			return $result;
		} else {
			return json_encode(array('OMG'=>" Ops .. Cant access the page !"));
		}
	...
	function getAutorizacion( Request $request)
	{

		if($request->ajax() == true && \Auth::check() == true)
		{
			$q=$request->input('q');
			$rows = $this->model->getAutorizacion($q);
			$items = array();
			...
			foreach($rows as $row)
			{
				...
				$ipsAsignada = "ipsAsignada";
				$fechaAutorizacion = "fechaAutorizacion";
				$items[] = array('id'=>$row->$id,'text'=> 'Número:'.$row->$numAutorizacion.' IPS:'.$row->$ipsAsignada.' Fecha:'.$row->$fechaAutorizacion);
			}
	...
	function getPacientes( Request $request)
	{
		...
		{
			$q=$request->input('q');
			$rows = $this->model->getPacientes($q);
			$items = array();
			...
			foreach($rows as $row)
			{
				...
				$nombres = "nombres";
				$apellidos = "apellidos";
				$items[] = array('id'=>$row->$id,'text'=> $row->$numDoc.' '.$row->$nombres.' '.$row->$apellidos);
			}
	...
	function getDiagnostico( Request $request)
	{

		if($request->ajax() == true && \Auth::check() == true)
		{
			$q=$request->input('q');
			$rows = $this->model->getDiagnostico($q);
			$items = array();
			...
			foreach($rows as $row)
			{
				...
				$items[] = array('id'=>$row->$IdDx,'text'=> $row->$IdDx.' '.$row->$Diagnostico);
			}
	...
	function getMedicamentos( Request $request)
	{

		if($request->ajax() == true && \Auth::check() == true)
		{
			$q=$request->input('q');
			$rows = $this->model->getMedicamentos($q);
			$items = array();
			...
			foreach($rows as $row)
			{
				...
				$items[] = array('id'=>$row->$id,'text'=> $row->$nombre);
			}
	...
	function getDiagnosticop( Request $request)
	{
		...
		if($request->ajax() == true && \Auth::check() == true)
		{
			$q=$request->input('q');
			$p=$request->input('p');
			$rows = $this->model->getDiagnostico($q,$p);
			$items = array();
	...
	function getMedico( Request $request)
	{
		...
		if($request->ajax() == true && \Auth::check() == true)
		{
			...
			foreach($rows as $row)
			{
				...
				$items[] = array('id'=>$row->$id,'text'=> $row->$idServ.' '.$row->$descripcion, 'idServ'=> $row->$idServ, 'descripcion'=> $row->$descripcion, 'codGrupo'=> $row->$codGrupo, 'mes'=> $row->$mes, 'codClasif'=> $row->$codClasif);
			}
			return json_encode($items);
		} else {
			return json_encode(array('OMG'=>" Ops .. Cant access the page !"));
	...
	function getTareas( Request $request)
	{
		...
		if($request->ajax() == true && \Auth::check() == true)
		{
			...
			foreach($rows as $row)
			{
				...
				$userid="userid";
				$entry_by="entry_by";
				$items[$row->$id] = array('id'=>$row->$id,'code'=> $row->$code, 'title'=> $row->$title, 'description'=> $row->$description, 'date'=> $row->$date, 'userid'=> $row->$userid, 'entry_by'=> $row->$entry_by);
			}
			return json_encode($items);
		} else {
			return json_encode(array('OMG'=>" Ops .. Cant access the page !"));
	...
	function getIdsavetask( Request $request)
	{
		...
		if($request->ajax() == true && \Auth::check() == true)
		{
			...
			//Log::info($task);
			$id = $this->model->insertRowTable($task , NULL, 'tasks','id');
	...
	function getIdupdatetask( Request $request)
	{
		...
		if($request->ajax() == true && \Auth::check() == true)
		{
			...
			$task['code']=$request->input('code');
			//Log::info($task);
			$id = $this->model->insertRowTable($task , $id, 'tasks','id');
	...
	function getIddeletetask( Request $request)
	{
		...
		if($request->ajax() == true && \Auth::check() == true)
		{
			...
			//Log::info($task);
			$id = $this->model->getIddeletetask( $id);
	...
	function inputLogs(Request $request, $note = NULL, $module=NULL, $task=NULL)
	{
		$data = array(
			'module'	=> $module,
			'task'		=> $task,
			...
			'note'		=> $note.' '.implode(' ',$request->input())
		);
	...
	{
		$info 		= $this->info;//$this->model->makeInfo( $this->module);
		//$master  	= $this->buildMasterDetail();
		$filter 	= (!is_null(Input::get('search')) ? $this->buildSearch() : '');
		//$filter 	.=  $master['masterFilter'];

		$results 	= $this->model->getCols( $filter );
		$fields		= $info['config']['grid'];
		$rows		= $results['rows'];
		//Log::info($rows);
		$content 	= array(
