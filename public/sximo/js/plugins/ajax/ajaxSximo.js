...
function ajaxInlineRemove(id,url)
{
		...
		if(confirm('¿Seguro desea eliminar estas columnas?'))
		{
...
function ajaxCopy(  id , url )
{
	...
	if(confirm('¿Está seguro de copiar la(s) columna(s)?'))
	{
...
{
	var datas = $( id +'Table :input').serialize();
	if(confirm('¿Seguro desea eliminar las columnas?')) {
...
function notyConfirm(id, url)
{
	...
	var n = noty({				
		text: '¿Seguro desea eliminar las columnas?',
		type: 'Confirm',
		timeout : 50,
		layout: 'topCenter',
